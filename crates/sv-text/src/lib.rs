...
//! for the optional LLM formatting pass, the prompt and checks for edit mode, and the one-line
//! preview the pill shows afterwards. Everything here is pure and fast, because it sits on the
//! latency path of every dictation.
#![forbid(unsafe_code)]
